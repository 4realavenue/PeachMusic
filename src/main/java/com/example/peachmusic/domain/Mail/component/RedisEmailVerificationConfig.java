...
public class RedisEmailVerificationConfig {
    private static final long EXPIRATION_TIME = 3 * 60; // 3분
    ...
    public void saveVerificationCode(String email, String code) {
        redisTemplate.opsForValue().set(email + ":code", code, EXPIRATION_TIME, TimeUnit.MINUTES);
        redisTemplate.opsForValue().set(email + ":verified", "false", EXPIRATION_TIME, TimeUnit.MINUTES);
    }
    ...
    public void verifyEmail(String email) {
        redisTemplate.opsForValue().set(email + ":verified", "true", EXPIRATION_TIME, TimeUnit.MINUTES);
    }
